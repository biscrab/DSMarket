...
const Review = ({item}) => {
    ...
    const deleteReview = () => {
        axios.delete("http://13.124.26.107:9095/api/comment", JSON.stringify({contents: item.contents, memberEmail: item.memberEmail}), config)
    ...
    return(
        ...
        <S.ReviewLi>
            <div>
            <S.RProfile src={profile}></S.RProfile><span>{item.memberEmail}</span><S.Span>{item.contents}</S.Span>
            </div>
